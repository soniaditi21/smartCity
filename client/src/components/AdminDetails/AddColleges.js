...
import { addCollegeDetail } from "../../service/itemApi";
...
function AddColleges() {
  ...
  const addItem = async () => {
    ...
    let response = await addCollegeDetail(item);
    if (!response) {
        ...
        <div className="rootContainer">
          ...
          <div className="mainContainer">
            ...
            <div className="totalQuantityContainer">
              <Typography style={{ fontSize: "30px", color: "white" }}>
               Total colleges added: 
               </Typography>
               <Typography style={{ fontSize: "30px", color: "white" }}>[ {colleges.length} ]</Typography> 
                ...
                <div className="table">
                {
                  colleges.length?(
                    colleges.map((college) => {
                      return (
                        <div className="table-row" key={college.itemTitle}>
                        <div className="table-column bank">
                          <div className="">{college.itemTitle}</div>
                        </div>
                        <div className="table-column ifsc">
                          <div className="">{college.primaryAddress}</div>
                        </div>
                        <div className="table-column branch">
                          <div className="">{college.secondaryAddress}</div>
                        </div>
                        <div className="table-column bank-id">
                          <div className="">{college.contact}</div>
                        </div>
                    ...
                    <Box sx={{ width: "100%" }}>
                      <Typography>No college added</Typography>
                    </Box>
                  </div>
                  )
