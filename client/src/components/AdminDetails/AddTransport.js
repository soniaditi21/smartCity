...
import { addTransportDetail } from "../../service/itemApi";
...
function AddTransport() {
  ...
  const addItem = async () => {
    ...
    let response = await addTransportDetail(item);
    if (!response) {
        ...
        <div className="rootContainer">
          ...
          <div className="mainContainer">
            ...
            <div className="totalQuantityContainer">
              <Typography style={{ fontSize: "30px", color: "white" }}>
               Total transports added: 
               </Typography>
               <Typography style={{ fontSize: "30px", color: "white" }}>[ {transports.length} ]</Typography> 
                ...
                <div className="table">
                {
                  transports.length?(
                    transports.map((transport) => {
                      return (
                        <div className="table-row" key={transport.itemTitle}>
                        <div className="table-column bank">
                          <div className="">{transport.itemTitle}</div>
                        </div>
                        <div className="table-column ifsc">
                          <div className="">{transport.primaryAddress}</div>
                        </div>
                        <div className="table-column branch">
                          <div className="">{transport.secondaryAddress}</div>
                        </div>
                        <div className="table-column bank-id">
                          <div className="">{transport.contact}</div>
                        </div>
                        <div className="table-column address">
                          <a href={transport.website}>
                          <div className="">website</div>
                          </a>
                        </div>
                      </div>
                    ...
                    <Box sx={{ width: "100%" }}>
                      <Typography>No transport added</Typography>
                    </Box>
                  </div>
                  )
