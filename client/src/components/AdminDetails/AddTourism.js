...
import { addTourismDetail } from "../../service/itemApi";
...
function AddTourism() {
  ...
  const addItem = async () => {
    ...
    let response = await addTourismDetail(item);
    if (!response) {
          ...
          <div className="mainContainer">
            <div className="headingContainer">
              <Typography style={{ fontSize: "30px", color: "white" }}>
                Total Tourism Places
              </Typography>
            </div>
            <div className='middleContainer'>
            <div className="totalQuantityContainer">
              <Typography style={{ fontSize: "30px", color: "white" }}>
               Total tourisms added: 
               </Typography>
               <Typography style={{ fontSize: "30px", color: "white" }}>[ {tourisms.length} ]</Typography> 
                ...
                <div className="table">
                {
                  tourisms.length?(
                    tourisms.map((tourism) => {
                      return (
                        <div className="table-row" key={tourism.itemTitle}>
                        <div className="table-column bank">
                          <div className="">{tourism.itemTitle}</div>
                        </div>
                        <div className="table-column ifsc">
                          <div className="">{tourism.primaryAddress}</div>
                        </div>
                        <div className="table-column branch">
                          <div className="">{tourism.secondaryAddress}</div>
                        </div>
                        <div className="table-column bank-id">
                          <div className="">{tourism.contact}</div>
                        </div>
                        <div className="table-column address">
                          <a href={tourism.website}>
                          <div className="">website</div>
                          </a>
                        </div>
                      </div>
                    ...
                    <Box sx={{ width: "100%" }}>
                      <Typography>No tourism added</Typography>
                    </Box>
                  </div>
                  )
